    ...
    <script>
        $(document).ready(function(){ 
            $(".editar").click(function(){ 
                ...
                var monto = $('#monto').val();  
                var fecha = $('#fecha').val(); 
                var presupuestoGeneral = $('#presupuestoGeneral').val();
                console.log(idPLV);
                Swal.fire({
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-heading text-center">
                        <br>
                        <h2 style="color:black">Editar Presupuesto para el Lavado del Vehículo</h2>
                        <br>
                    </div> 
                    <div class="card-body">  
                        <div class="form-group row"  style="display:none">
                            <label for="idPLV" class="col-md-4 col-form-label text-md-right" style="color:black">{{ __('Código: ') }}</label> 
                            <div class="col-md-6">
                                <input id="idPLV" type="text" class="form-control{{ $errors->has('idPLV') ? ' is-invalid' : '' }}" name="monto" value="{{ $presupuestoLV->idPLV}}" readonly autofocus>
                            </div>
                        </div> 
                        <div class="form-group row">
                            <label for="fecha" class="col-md-4 col-form-label text-md-right" style="color:black">{{ __('Fecha: ') }}</label> 
                            <div class="col-md-6">
                                <input id="fecha" type="date" class="form-control{{ $errors->has('fecha') ? ' is-invalid' : '' }}" name="fecha" value="{{ $presupuestoLV->fecha }}" required autofocus>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="monto" class="col-md-4 col-form-label text-md-right" style="color:black">{{ __('Monto Establecido: ') }}</label> 
                            <div class="col-md-6">
                                <input id="monto" type="text" class="form-control{{ $errors->has('monto') ? ' is-invalid' : '' }}" name="monto" value="{{ $presupuestoLV->monto }}" required autofocus>
                            </div>
